...
| A street sign you can actually read | The name of the road you are on held for **four seconds** — about two blocks at speed, so you caught it only if you happened to be looking at that corner of the screen when it appeared. Nine now, which leaves time to glance down after the corner rather than during it; nothing queues up, because a new street replaces the label and restarts the clock. And it was **cyan at night and a dark teal by day**: two colours to keep right instead of one, and the daylight half had been chosen for a pale ground that is now dark sea-green, so it was heading for teal on teal. It is **gold with a light blue edge in both themes** — the gold is far from anything the game draws under it, the ring separates it from a sunlit facade where gold alone washes out, and a soft dark drop underneath carries the one case a light ring cannot, which is light on light. Eight `text-shadow` offsets rather than `-webkit-text-stroke`, which thins hairlines at some sizes. `tests/nav.mjs` measures the hold **end to end in simulated seconds** — from the label changing to it going away, stopping the car dead so a coast across the next junction cannot silently turn one hold into two — and holds the colours against `--gold` and `--cyan` rather than against hexes typed into the test. That file also **got a verdict**: for its whole life it printed a report and exited zero, the same hole `hud.mjs` and `poi.mjs` were pulled out of |
