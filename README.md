...
data/belgrade.js  the bundled offline city — real Stari grad, Belgrade
tools/buildcity.py  rebuilds that city from the captured map data
tests/          the replay test, and the captured map data it replays
...
- **A 200 km city, loaded before you drive.** The whole road network arrives while the loading
  ...
  primary and secondary roads — across **100 km in every direction**. Asking for every street in a
  box that size is gigabytes; the roads you'd actually cross a country on are a few thousand, and
  they're what gives the place a horizon. If the wide request is refused it retries at 36 km, then
  18, then 9, so you get the biggest world that server would give you — and the rung under the big
  ask is 36 km exactly, so a refusal lands on the world the game had before it asked. Once you're driving
...
| Daylight is rush hour | Ten times the cars while the sun is up — a bright empty city reads as an abandoned one, where dusk can carry a quiet street on atmosphere. It is one number, because the machinery already existed: the top-up fills towards a cap a few cars at a time and the 780 m cull empties it from behind. Measured at 255 cars, the per-frame update goes from 0.18 ms to 2.6 ms of a 16.7 ms budget, the drawing costs the same as dusk (see below) and the frame rate holds at 60. The refill rate stays at five a tick, though — raising it to ten to fill the rush hour faster cost four frames a second at DUSK, where cars are culled and replaced constantly as you drive, so the burst is reserved for a deficit big enough to be a standing start or a theme change |
...
| The wide skeleton | One `arterials` request over a **200 km box**: `motorway\|trunk\|primary\|secondary` and their links, plus place nodes for the district banner. Deliberately **not** the full drivable set — residential lanes are the overwhelming majority of a city's ways and putting them back is the difference between twenty megabytes and a query the server refuses outright. Two clauses, same shape as the streets query, for the same reason. Tried at 100 km, then 36, then 18, then 9 under one shared deadline, so a server that won't answer for the big box still gives you the biggest world it will — and the box grows with the SQUARE of the radius, so 100 km asks for nearly eight times the area 36 did and there are networks that will not carry it. The rung under the big ask is 36 km exactly, so a refusal lands on the world the game had before it started asking. The 100 km rung is capped at 30 s, five fewer than the 36 km rung used to get, which leaves 18 s for the fallback against the eight seconds it is known to take: what asking costs, when the answer is no, is one bounded wait and then the same city |
...
**bundled offline city**: Stari grad, the old town of Belgrade, shipped in `data/belgrade.js`
and captured from OpenStreetMap by the in-game log button: 1,161 real streets and squares in the
detailed centre, 3,782 real buildings and parks, and its own arterial skeleton reaching **15 km**
in every direction — a 30 km world, not the pocket the first version bundled. It is the same data
the online path builds from, so nothing downstream can tell the difference. A generated grid is still underneath it, in case the
