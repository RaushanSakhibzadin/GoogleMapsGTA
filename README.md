...
- **The city opens at 5.4 km, and keeps growing.** The first 1.8 km tile arrives fast, then its
  eight neighbours come in while the loading screen is still up — nine tiles, nine times the area.
  From there it streams: get within ~500 m of the edge and the next district is fetched in the
  background and folded into the live world, grid, collision, radar and all. Tiles you've left far
  behind are recycled to make room, so it keeps going however far you drive. Packets, traffic and
  pedestrians spawn out in the new districts, and delivery contracts reach further as more arrives.
...
- **A widening sweep for landmarks, while it loads.** The opening download is 1.8 km across, and
  ...
  on the road, nothing but small neighbouring tiles is ever fetched.
...
| Streaming | Tile (i,j) is the 1.8 km square centred on (i·1800, j·1800) in local metres, so tile (0,0) is the opening area and neighbours abut it exactly. The eight around it are pulled during loading, so the city starts at 5.4 km. **World size comes from the tiles loaded, never from the geometry inside them** — Overpass returns the full shape of anything merely touching the box it was asked for, and one riverbank way once stretched the world to ±150 km, which shrank the city to a speck on the radar and blew the collision mask up to millions of cells. Overhanging features are simply drawn and clipped. One tile in flight at a time, with a cooldown and per-tile backoff |
| Recycling | Twenty tiles are held at once — the opening ring plus room to drive — and the ones furthest behind you are dropped to make room, so you can drive indefinitely without the collision mask or the pre-rendered map growing without bound. Every feature carries the tile it arrived with, so a tile leaves with its own scenery, and what remains is re-indexed from scratch. The look-ahead only ever wants nine tiles, so what's in play is never what gets dropped |
