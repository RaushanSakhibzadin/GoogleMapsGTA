...
| One yellow, not three | The wanted stars are meant to match the objective arrow, and had been "fixed" twice: white at 16% alpha for the unlit ones, which read as grey ticks, then a hand-typed `#ffd21a` that came out orange beside the arrow in daylight. Both times the fault was the same — the colour written out again somewhere else. There is one constant now, `GOLD` in `js/util.js` and `--gold` in the stylesheet, and the arrow, the marker, the radar blip and both themes' stars all read it. `tests/daynight.mjs` holds the stylesheet, the canvas and the stars against each other rather than against a number typed into the test, which would only have been a fourth copy. Daylight takes its contrast from a dark outline instead of a darker star: the thing that needs to be dark is the edge, not the fill — darkening the star itself is what turned it orange |
| Four shifts | Taxi, police, fire brigade and ambulance, taken by pulling up at the depot that hires for it and pressing a button carrying that vehicle. **The depots are real**: `amenity=taxi`, `police`, `fire_station` and `hospital`, riding the same tag-indexed sweep that already goes out to 18 then 45 km looking for a hospital — measured on the checked-in capture, the 1.8 km the opening ring covers contains neither a fire station nor a taxi rank. Nothing is placed by the game, so a city without one has no work of that kind in it. **Four generators, not four subsystems.** Everything downstream already worked in terms of a target, a clock and a reward, and reads one MISSION object; on the way, the **seven** places that each asked their own version of "where is the job" — world marker, map dot, map face, radar blip, radar rim pointer, edge arrow, chase-view beacon — became one `missionGoal()`, which is what made a burning building and a fleeing car possible as goals at all. TAXI takes a fare out of the crowd already walking the pavements rather than dropping a marker on a road point, which looks like a passenger right up until you arrive at an empty kerb; they stand still while they wait and the street is one person emptier once they are in. AMBULANCE ends at an actual hospital. FIRE burns a real building through the particle system the explosions already use — drawn by **both** renderers — and the number climbs back while you are away, so driving off is losing ground rather than pausing. POLICE marks a car out of the traffic that was going about its business a second ago; beaten below the threshold it gives up, which covers arrest and destruction both. The armoured car is one line in `hurtPlayer`, the single door every damage source already goes through: 18 against a courier's 40. `tests/jobs.mjs` targets the ways a shift half-works rather than fails — the fare walking off, the fire going out on a drive-by, the chase target being a car that was already gone. `tests/hud.mjs` raises the button by hand and measures it at seven viewports, because `display:none` is skipped by the overlap check and it would otherwise be the one new control never tested for landing on a thumb pad |
| Each shift's own words | Reported from play: the ambulance said **PASSENGER ABOARD**. Same fault as PACKAGE LOST the round before, and the same cause — the choice was a ternary, "courier or not", which has no room for a third answer. Three tables now (`BOARD_TOAST`, `DONE_TOAST`, `FAIL_TOAST`), each falling back to the courier's line, and one word per language for whoever is in the car: the English is "patient" from the pickup objective to the drop-off toast. Then the whole thing was swept in all ten languages, which is where the interesting part was — `toast.delivered` and `toast.droppedOff` were the **same string** in Spanish, Italian, Portuguese and Chinese, so the courier's "DELIVERED" and the taxi's "DROPPED OFF" had quietly collapsed into one word in four locales. Every locale was written by copying the block above it, which is exactly how that happens. `tests/jobs.mjs` walks all three carrying shifts through a full run and reads what was on screen, then asks every language for its version of five moments and fails on any two that come back identical |
| The runaway starts a long way off | Reported as too easy, and it was. The target was marked out of the traffic already on the street — but traffic only exists inside the cull radius, about **260 m**, so "the nearest car in the band" was the one in your windscreen: measured on the old build, 122, 132 and 127 metres. It is now put on a road **300–900 m** away and pushed into the traffic list, where it drives like any other car; measured on the new build, 476 to 878. Three things had to change with it, and each was a separate bug the distance exposed. The cull deletes traffic past the edge of the screen, so the runaway is exempt and given a **1500 m leash** of its own, the same arrangement the police cars have always had. The traffic **cap** truncates the list with `traffic.length = cap`, and the runaway is pushed onto the end — precisely what a truncation eats, which deleted the target a quarter of a second after the shift handed it out; it is counted separately and lifted out of the way of the trim. And the spawn band must not use `missionReach`, which widens with the streamed map: it put the target 2.9 km away, outside its own leash, so the pursuit announced that it had got away before you had moved |
| Losing it pays nothing | The other half of making it distant. "Gone from the traffic list" was read as "stopped", so the reward landed the moment the target was culled — measured on the old build, shoving it 4 km away paid $600 and counted a completed job. A wreck still counts as a stop; merely getting away is `failPursuit()`, which pays nothing and says so. Without this the harder version of the shift would have been the easier one to farm, since a target that starts 300 m out is culled the instant you fall behind |
| Only a real smash brings the police | Reported from play: the wanted level went up for every scrape. The bar was 13 m/s of closing speed — doing 100 km/h into the back of a car doing 40 closes at about 17, so ordinary traffic was starring you; a head-on with the same pair closes at nearly 40. The bar is 18 now, the damage threshold is untouched (a bump still costs paint, it just no longer costs a star), and ramming the car you were **sent** to stop no longer earns you a wanted level of your own — on a police shift the runaway is an ordinary traffic car as far as the collision code knows, so doing what the objective asked was being punished. The test's middle run is the one that matters: a light tap never starred you and a head-on always will, so a test of only those two passes on the old bar as happily as the new one — it did, first time out. The discriminating run lands **between** the two bars, and is asserted to, by recovering the closing speed from the damage ledger |
| The depot's counter is on the street | Reported from play: "fire stations have no roads to come close to." A **log from the game itself** settled it — the Save Logs button ships the raw OpenStreetMap replies, so the exact session could be replayed. On the station the report came from, Ватрогасни савез Београд at world (975, −180), the nearest way of **any** kind is 60.5 m and the nearest one you may drive on is the same one; `JOB_RANGE` is 22. The depot was unreachable by car, and the log has the player stopped 37 m short of it, off the tarmac, doing 9 km/h — which is the off-road crawl. Not a fire-station problem either: it is what happens to any landmark set back in its own yard, and stations, hospitals and depots are exactly the buildings that are. A depot now offers its work from two places, itself and its **gate** — the nearest drivable point to it — so pulling up on the street outside is enough, which is the only thing a car can do. The search is bounded to the roads near the depot rather than `nearestRoadPoint`'s widening ladder, whose last rung is a scan of all nineteen thousand drivable ways in Belgrade and does not belong anywhere near a ten-times-a-second tick; the answer is kept per streaming step, because a gate worked out before this district's streets landed is a worse answer cached for the session |
| A fire is across town | Also reported: the fires were too close to the station. They were — the building was the **nearest** one at least 60 m off, handed out while you stand at the station, so every call was sixty metres from the front door. Three hundred to nine hundred metres now, and picked at **random** from that band rather than nearest-first, because nearest-first inside a band puts every fire at exactly the minimum — the same complaint with a different constant. Measured on the old build the test saw six identical distances; on the new one, six different ones between 539 and 824 m |
| The fire engine | A saloon in red is a saloon in red, so the fire shift changes the **vehicle**: 7.4 m long, 2.5 wide, 2.7 tall, and a body built out of the car's own eight corners instead of the car. The glasshouse in the middle, the tapered bonnet and the boot are exactly what an appliance does not have — it is a tall slab of bodywork with a small cab on the front, a white band and four locker shutters down each flank, a ladder with nine rungs along the roof, three axles because two on a seven-metre wheelbase reads as a limousine, and the same alternating bar the patrol cars carry. All of it in fractions of whatever body it is given, so it is the shift's dimensions that make the truck a truck. The stock length and width are kept when they are replaced: `makeCar` randomises them, so restoring "a default" would hand back a different car every time you clocked off. `tests/livery.mjs` reads it as a whole vehicle rather than a marking — 880 changed pixels against 138 to 240 for a livery, and 11,920 in the chase view against a noise floor of nought |
| The passenger gets out | Reported from play: the fare vanished at the drop. Taking somebody out of the crowd to carry them is only half of it — a ride that ends without putting them back deletes a person from the city every time you get paid, and the kerb you pulled up at is empty. So every way a ride can end now lets them out: delivered, out of time, wrecked, busted, or the shift changed underneath them. The one exception is the ambulance, where the casualty is delivered *into* the hospital. Two things about it are less obvious than they sound. They are put on the **pavement**, offset across from the drop pin — the pin is on the centreline and the centreline is the middle of the road — and offset from the *point* rather than snapped to the road's nearest node, because OpenStreetMap ways run hundreds of metres between nodes and snapping had them step out of the cab at the far end of the street. And the side of the road is read **relative to the way they face**, which is how `pedWalkPoint` reads it: getting that backwards put them on the correct pavement and then walked them straight across the carriageway to the other one on their first step — visible in the test as a lateral 5.5 m from a 8.0 m offset. `tests/jobs.mjs` measures the distance across the centreline rather than a plain radius from the pin, so walking a couple of metres along the kerb is not mistaken for stepping into the road |
| The car looks like the job | Paint alone never carried it: white is an ambulance, a police car and half the traffic. One word on the car — `livery` — is read by both renderers, so a shift dresses the car in the top-down view and the chase view at once. **Police** reuses the livery the patrol cars already wear, Sillitoe chequer and lightbar, so clocking on makes you look like the cars you drive alongside. **Ambulance** takes a red cross and a red band down each flank. **Taxi** takes a nine-column chequer and a roof sign. Both new liveries are built the way the police one is — from the car's own eight corners in 3D, so they land on the detailed body up close and the two-box version at distance and ride the pitch and roll with the paint. **The roof is where the work is**: the chase camera looks along the car, so the flanks are nearly edge-on and the roof is most of what you see of your own vehicle — measured, the taxi's roof sign is worth about fifty screen pixels at driving distance and its flank chequer none of them, which is why the cross and the sign go on top, exactly where a real one puts them and for the same reason. A side effect worth having: the body shop no longer resprays a city vehicle, because an ambulance coming out of the garage hot pink still wearing its cross is a joke that does not survive being seen twice |
| The runaway is red | It was drawn in `#3fa2ff` — the same blue as the patrol blips and the police station — which put the one car you are chasing in the livery of the people chasing it. On a police shift half the radar is already blue. One line in `missionGoal()`, which is what the map dot, the map face, the radar blip, the rim pointer, the edge arrow and the chase-view beacon all read |
| `tests/livery.mjs` | Every reading is a **difference**: the same frame rendered twice, once as it is and once with `livery` nulled and nothing else touched, so what moved is the markings and a build without them scores zero. The two renderers need two instruments. The flat view is deterministic, so it is an exact pixel diff plus the mean colour of what changed — a first version counted pixels by colour class instead and had to be thrown away, because the markings are two pixels across at that zoom, a third of them sit on a classifier boundary, and the count swung between 22 and 55 on an unchanged build. The chase view will not hold still at all: it streams one cell of city per frame, so two consecutive frames of a **parked** car differ in 150,000 pixels, and the reading there is a box around the car compared against a third frame taken with nothing changed — markings move 400–800 pixels in that box, the do-nothing pair moves 0–120. The car is teleported to the same pose before every reading, because the camera distance follows the car and letting it drift moved one measurement by a factor of fifteen |
| What was lost | An ambulance that ran out of time said **PACKAGE LOST**. One failure line was written for the courier run and then inherited by every shift that has a clock, so the taxi lost a parcel too. Three lines now, in all ten languages — the fare walked out, the patient was lost, the package was lost — and `tests/jobs.mjs` asks for them through the **keys** rather than the English, so it holds in each. The passenger is part of the message: an ambulance run ends at the hospital or it does not end well, and either way the casualty does not hop out and stroll off, while a taxi fare does. Writing the test found something else — every mission ends by asking for the next one on a **timer**, and clocking on at a depot asks for one too, so failing a run and walking straight into a fire station queued two of them and the stale one landed a couple of seconds into the new shift and re-rolled the job you had just been handed. All nine call sites go through `scheduleMission()` and a sequence number now, which `clearMission` bumps to void a pending call it did not schedule |
| Clocking on clears the heat | Also from play. Signing for a city vehicle at the counter is not something that happens with a patrol car still on your tail, and a pursuit that outlives the shift it started in follows you into every shift after it. `clearHeat()` drops the stars, dismisses the cars — five of them still ramming an ambulance is the same problem in a different colour — and resets the bust timer, and `doRespawn` uses the same function instead of its own copy of two thirds of it |
| Finding yourself on the map | Reported: you cannot always find yourself on the big map. Two things, and they are separate. **Panning is how you lose yourself in a 72 km city** — two drags out and the car is off the edge with nothing to say which edge — so there is a locate button under the thumb, stacked above the close button on the same edge so the pair reads as one control column. It re-centres and **changes nothing else**: the zoom you pinched to is the reason you were panning, and a button that quietly called `mapFit()` with no argument would put the car in the middle while throwing that away (measured on a build that does exactly that: 0.72 → 0.12). **And the arrow was too small to spot** — a 16×13 device-pixel arrowhead, five points across on a phone, competing with a whole city. It is 26×21 now and the outline went with it, which takes its white area from about 41 pixels to 113. It deliberately does **not** scale with the zoom, unlike the landmark faces beside it: those grow because zooming in is asking to read a street, while the car has to be equally findable at every zoom — most of all at the far end, where the reason you are looking is that you have lost track of where you are. `tests/mapzoom.mjs` drags the map 3.3 km off the car and presses the real button through the browser's own hit test, then asserts the scale unchanged to five decimals; the arrow is measured as white AREA rather than a bounding box, because a rotated arrow's width and height are a fact about which way you were pointing |
| A caption is its building's colour | Shop and building names were painted in two house colours — a mid blue, and yellow for anywhere you eat — chosen to read against masonry in general. They are the colour OpenStreetMap gives each building now: `building:colour` where a mapper set one, otherwise the material it was classified as, which is the same value the wall itself is drawn from. **The obvious version of this is invisible.** Letters in exactly their own wall's colour are no letters, so `signInk` keeps the hue and moves the LUMA clear of the wall **as drawn** — lighter than a dark wall, darker than a pale one, which is the direction real signage goes anyway. Against the wall as drawn matters: dusk multiplies a material by 0.17 and pulls it towards a violet ambient, daylight multiplies by 0.66, so the same building needs the push in opposite directions in the two themes; reading the theme's own `wallT` makes that fall out instead of being a rule per theme. Darkening scales (exact hue); lightening mixes towards white, because scaling cannot start from black — a black building came out with black letters on a black wall, gap zero — and clips the strong channel first, which turns a scaled-up brick orange. **Both themes' inks ride in the mesh**, six floats a vertex on the smallest mesh in the world, with one uniform picking between them: the colour is per building so it cannot be a uniform, and it cannot be one baked colour either, because a cell is built once and cached while pressing N has to move every sign in the city on the same frame. That also keeps the arithmetic in one place rather than restating the theme's wall transform in GLSL. `tests/sign.mjs` paints every signed building red, reads the frame, paints them blue and reads it again — one property apart — and separately holds `signInk` against the whole material palette in both themes. Against a build with a constant ink: the captions stop tracking the building. Against one with the colour but no push: **worst gap 0** |
| The city in a script you can read | The UI was translated into ten languages and the city was not: a German, Japanese or English player driving Belgrade got Ђуре Даничића on the street banner and Скадарлија for the district, because those come off OpenStreetMap's `name` tag, which is the **local** name. Counted on the checked-in Belgrade capture rather than guessed at: of 847 named streets, `name:sr-Latn` covers **847**, `int_name` 845 and `name:en` **five**. Nobody translates eight thousand proper nouns and nobody ever will — but every single way carries the name in another **script**. So this does not translate anything; it picks the spelling the reader can read, in **script-major order**: the reader's own language first, then the local name, then a romanisation. That order is why a Russian player keeps Serbian Cyrillic — readable to them, and the truer name — while a German player gets the Latin form of the same street; a change that romanised everything for everybody would be a regression, and would sail through a check that only counted Latin. The proper romanisation is looked for **before** `int_name`, because they are not the same string: `Đure Daničića` against `Djure Danicica`. The tag walk that finds it runs once and remembers the key — these objects carry 150 `name:*` keys each and a city has eight thousand ways, so per-road it would be over a million comparisons; with the hint, parsing 842 roads costs 2–4 ms. The city's own name is asked of Nominatim with `accept-language`, which cannot disturb the radio because the country code comes from the structured address, not the prose. `tests/names.mjs` measures coverage over the whole real city in all ten languages and separately pins seven picker decisions the coverage number cannot isolate. Against a build with the helper present but unwired: **0%** for every Latin-reading language against 100%, Нишка against Niška |
| GHOST is behind a word now | The supporter perk was a switch sitting in the menu next to the Patreon ask, free to anyone who scrolled that far — which is a perk nobody has to back anything to get, and there was no moment where supporting the thing gave you something you did not have thirty seconds earlier. The word goes in the Patreon post; until it is typed the switch does not exist. **The gate is at the read, not only at the switch**: `GHOST = PERK && …`, because every player who ever used the free toggle still has `vm_ghost=1` in their browser and would otherwise keep the perk for ever — the lock would have applied to new players only, the one group that does not need it. That stale flag is cleared on load too, or the gate would hold for one session and then hand the perk over by itself. The unlock is stamped with the word (`w:kalemegdan`), so **changing the word re-locks everyone who used the old one** instead of leaving last season's word working on every phone that ever typed it. Case, spaces and stray padding are all forgiven, because it gets typed on a phone with autocorrect fighting back. **What this is:** a courtesy lock. The game is static files, the source is public, and `PERK_WORD` is in plain text in `js/game.js` — anyone who opens the console can read it or set the flag themselves, and hashing it would change nothing, because the code doing the compare is equally readable and equally editable. What it buys is that the perk is no longer sitting in the menu for every passer-by, and that the word is worth something to the people who paid for it; what plaintext buys is being able to test the real path with the real word. `tests/harness.mjs` reads `PERK_WORD` out of the game rather than copying it, and `tests/ghost.mjs` drives the real box and the real button — wrong word first, because a gate that opens for everything is not a gate |
| A street sign you can actually read | The name of the road you are on held for **four seconds** — about two blocks at speed, so you caught it only if you happened to be looking at that corner of the screen when it appeared. Nine now, which leaves time to glance down after the corner rather than during it; nothing queues up, because a new street replaces the label and restarts the clock. And the colour took three goes. Cyan at night and dark teal by day was two colours to keep right instead of one, and the daylight half had been chosen for a pale ground that is now dark sea-green. Gold with a light blue ring fixed that and created a second problem: at 13px the ring is a large fraction of each stroke, so it read as a **blue haze** rather than an outline and the word came out looking blue-green — reported exactly that way. It is **gold with a hard near-black edge** now, the same in both themes: an outline separates the gold from what is behind it without competing for the colour, and **nothing is blurred** — a soft drop underneath would be a second aura doing the job the outline already does. 15px rather than 13, because the road you are on is read from the corner of the eye at speed. Eight `text-shadow` offsets rather than `-webkit-text-stroke`, which thins hairlines at some sizes. `tests/nav.mjs` measures the hold **end to end in simulated seconds** — from the label changing to it going away, stopping the car dead so a coast across the next junction cannot silently turn one hold into two — and holds the banner to the SHAPE of the rule rather than to a hex: the fill against `--gold`, the stylesheet's own constant; the edge to being dark and repeated (an outline, not a drop), unblurred (which a screenshot cannot tell from a soft one), and carrying no blue. Against the ringed build all four fail and `gold` still passes — the fill was already right, the edge was not. That file also **got a verdict**: for its whole life it printed a report and exited zero, the same hole `hud.mjs` and `poi.mjs` were pulled out of |
| The job pings | The objective had a dot on the radar and a dot with an emoji on the big map, and neither moved — so on a city map carrying a hundred landmarks the one place you are actually driving to looked like all the others. It is a sonar ping now, **two rings half a period apart** so there is always one on the way out and one on the way in; one ring alone leaves a dead beat every cycle that reads as a stutter rather than a pulse. The rings are sized off `mapFaceSize`, so the ping scales with the zoom exactly as the marker under it does, and coloured off the mission stage — pink to collect, gold to deliver, the pair the arrow already uses. **It needed a second canvas.** `drawBigMap` walks every road in the world and is deliberately called only when the map MOVES; an expanding ring is a per-frame job, so it goes on `#bigmapFX` above the city with `pointer-events:none` — a full-screen sheet over the canvas the pan and pinch handlers are bound to is a map you cannot drag, and it looks perfectly fine in a screenshot. On the wall clock, not `SIMT`, which stops while the map is open. `tests/pulse.mjs` drives the phase rather than sampling the clock, and **counts calls to `drawBigMap` rather than diffing pixels**: the naive implementation that redraws the city sixty times a second paints the same deterministic picture and comes back byte-identical, so the pixel check that was supposed to catch it caught nothing |
| Ground you would swim in | The daylight soil between the roads was a bone grey `#cfc9bd`, and was asked to become a dark blue-green — "sea, grass and river at the same time". It is `#1c5249` now: hue 170°, which is the midpoint between green and cyan and so reads as all three, at a fifth of full lightness because this is the one surface in the game that is never lit, shaded or textured and at any lighter value it becomes the brightest thing in the frame. **One string, three renderers** — `SH_GND_F` deliberately does not relight flat ground (it solves to `k = 1.0`), so the top-down view, the software 3D and WebGL all paint the value as written, and `tests/ground.mjs` holds the top-down frame to it byte for byte and the chase frame to the same channel ratios at any brightness, since the terrain under the grass rolls. The **thumb controls had to change with it**: they were a dark wash with a dark glyph, correct against the grey they were designed on and invisible against dark grass — DRIFT and the day/night toggle came out as empty outlines the moment the car left a built-up street. Light fill, dark glyph, dark rim, so the button carries its own contrast over either. The **map is drawn on the same soil** — one named constant, `DAY_SOIL`, because the first version of this changed the world and left the map on the old grey, so opening it in daylight went from a dark green city to a sheet of paper. The radar reads `mapBg` too, so both go together, and white minor roads with pale gold arterials separate better on dark ground than either did on grey; the map's three daylight label overrides were deleted rather than re-tuned, because the stylesheet's night rules are now the right ones for both themes. The night is untouched, and the test says so |
