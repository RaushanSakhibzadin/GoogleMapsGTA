...
- **A 400 km city, loaded before you drive.** The wide road network arrives while the loading
  screen is up and is never fetched again. It comes at two levels of detail: every lane and service
  road across the 5.4 km you start in, and the **arterial skeleton** across **200 km in every
  direction**. Asking for every street in a box that size is gigabytes; the roads you'd actually
  cross a country on are a few thousand, and they're what gives the place a horizon. The ask narrows
  as it widens, in one request — the motorway network over the full 200 km, primaries over 100, the
  full arterial set over the 36 km the drivable mask covers — because area goes with the square of
  the radius and a flat 200 km ask is four times the ground of a 100 km one that already came back
  as 45 MB. If the wide request is refused it retries at 100 km, then 36, 18, 9, so you get the
  biggest world that server would give you, and each rung is a world the game has actually shipped
  with rather than a round number. **Streets keep streaming as you drive**, a 1.8 km tile at a time:
  the skeleton is arterials, and the street you live on is not an arterial. Packets, traffic and pedestrians spawn wherever you are, and
