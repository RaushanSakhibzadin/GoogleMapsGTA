...
| Daylight is rush hour | Ten times the cars while the sun is up — a bright empty city reads as an abandoned one, where dusk can carry a quiet street on atmosphere. It is one number, because the machinery already existed: the top-up fills towards a cap a few cars at a time and the 780 m cull empties it from behind. Measured at 255 cars, the per-frame update goes from 0.18 ms to 2.8 ms of a 16.7 ms budget and the frame rate holds. The refill rate stays at five a tick, though — raising it to ten to fill the rush hour faster cost four frames a second at DUSK, where cars are culled and replaced constantly as you drive, so the burst is reserved for a deficit big enough to be a standing start or a theme change |
